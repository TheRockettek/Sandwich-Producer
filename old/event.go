...
func (s *Session) AddHandler(handler interface{}) func() {
	...
	if eh == nil {
		s.log.Error().Msg("Invalid handler type, handler will never be called")
		return func() {}
	}
...
func (s *Session) onInterface(i interface{}, eventName ...string) {
	switch t := i.(type) {
	case *Ready:
		for _, g := range t.Guilds {
			setGuildIds(g)
		}
		s.log.Info().Msg("Session is ready")
		s.onReady(t)
	case *GuildCreate:
		setGuildIds(t.Guild)
	case *GuildUpdate:
	...
	err := s.OnInterface(i)
	if err != nil {
		s.log.Debug().Err(err).Msg("error dispatching internal event")
	}
}
