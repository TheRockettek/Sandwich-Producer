...
func (s *Session) onReady(r *Ready) (err error) {
	...
	s.Unavailables = make(map[string]bool)
	for _, g := range s.Ready.Guilds {
		s.Unavailables[g.ID] = g.Unavailable
	}
...
func (s *Session) OnInterface(i interface{}) (err error) {
	...
	case *GuildCreate:
		...
		ok, val := s.Unavailables[t.Guild.ID]

		if ok && val {
			s.log.Info().Str("id", t.Guild.ID).Msg("guild was previously in unavailables")
			if !t.Guild.Unavailable {
				s.log.Info().Str("id", t.Guild.ID).Msg("guild is now available")
				delete(s.Unavailables, t.Guild.ID)
			}
			return nil
		}
