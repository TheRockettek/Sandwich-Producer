...
type Marshallers struct {
	sync.Mutex
	Marshalers map[string]func(Event) (bool, interface{})
}
...
type MarshaledGuild struct {
	...
	// A list of roles in the guild.
	RoleValues map[string]interface{} `json:"-"`

	// A list of the custom emojis present in the guild.
	EmojiValues map[string]interface{} `json:"-"`

	// A list of channels in the guild.
	// This field is only present in GUILD_CREATE events and websocket
	// update events, and thus is only present in state-cached guilds.
	ChannelValues map[string]interface{} `json:"-"`
}
...
func (m *Marshallers) addMarshaler(event string, marshaler func(Event) (bool, interface{})) {
	m.Lock()
	defer m.Unlock()
...
var marshallers = Marshallers{
	Marshalers: make(map[string]func(Event) (bool, interface{})),
}

func init() {
	marshallers.addMarshaler("READY", func(e Event) (bool, interface{}) {
		return true, e
	})
}
